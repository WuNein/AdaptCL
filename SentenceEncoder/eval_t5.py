...
def evaluate_single_checkpoint(
    ...
    def batcher(params, batch, max_length=None):
        ...
        if add_prompt:
            sentences = [' '.join(s) + ' Question: what can we draw from the above sentence?' for s in batch] # draw -> learn 85.19->85.10
        else:
            sentences = [' '.join(s) for s in batch] #84.18 without prompt
